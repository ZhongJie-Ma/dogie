...
class turn_on_robot
{
	...
	private:
		...
		ros::Subscriber Cmd_Vel_Sub,Recharge_Flag_Sub,Red_Vel_Sub,Security_Sub,joint_state_Sub; //Initialize the topic subscriber //初始化话题订阅者

		//速度话题订阅回调函数
		void Cmd_Vel_Callback(const geometry_msgs::Twist &twist_aux); 
        void joint_states_Callback(const std_msgs::Float32MultiArray arm_joint);       
        ...
        string usart_port_name, robot_frame_id, gyro_frame_id, odom_frame_id,robot_type,car_mode,JointStates_topic; //Define the related variables //定义相关变量
