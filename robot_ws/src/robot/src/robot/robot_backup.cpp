...
{
  turn_on_robot::supersonic distance_msg;
...
{ 
  ...
  */  
  ...
  if(count >= RECEIVE_DATA_SIZE) count = 0;
  if(count2 >= Distance_DATA_size) count2 = 0;
  if(count3 >= AutoCharge_DATA_SIZE) count3 = 0;
  ...
  if(((Receive_Data_Pr[0] == Distance_TAIL || Last_Receive == FRAME_TAIL)&& Receive_Data_Pr[0] == AutoCharge_HEADER)||count3>0)
    count3++;
  else
    count3=0;
  ...
  if(((Receive_Data_Pr[0] == AutoCharge_TAIL || Last_Receive == FRAME_TAIL)&& Receive_Data_Pr[0] == FRAME_HEADER)||count>0) //Ensure that the first data in the array is FRAME_HEADER //确保数组第一个数据为FRAME_HEADER
    count++;
  else 
    count=0;
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  distance_publisher= n.advertise<turn_on_robot::supersonic>("Distance", 10); //创建超声波距离话题发布者
