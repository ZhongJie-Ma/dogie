...
bool turn_on_robot::Set_Charge_Callback(turtlesim::Spawn::Request &req,turtlesim::Spawn::Response &res)
{
    ...
    if(round(req.x)==1)
      Send_Data.tx[1] = 1,AutoRecharge=1;
    else if(round(req.x)==2)
      Send_Data.tx[1] = 2,AutoRecharge=2; 
...
void turn_on_robot::joint_states_Callback(const std_msgs::Float32MultiArray arm_joint)
{
      //  printf("joint_states_Callback \n");
       if(fabs(last_A-arm_joint.data[0])>0.01||
        fabs(last_B-arm_joint.data[1])>0.01||
        fabs(last_C-arm_joint.data[2])>0.01||
           last_grap!=arm_joint.data[3] )
      {
        last_A=arm_joint.data[0];
        last_B=arm_joint.data[1];
        last_C=arm_joint.data[2];
        last_grap=arm_joint.data[3];
      }
      else return;
      Send_Data.tx[0]=0xAA;
      Send_Data.tx[1]=((short)(arm_joint.data[0]*1000))>>8;
      Send_Data.tx[2]=(short)(arm_joint.data[0]*1000);


      Send_Data.tx[3]=((short)(arm_joint.data[1]*1000))>>8;
      Send_Data.tx[4]=(short)(arm_joint.data[1]*1000);

      Send_Data.tx[5]=((short)(arm_joint.data[2]*1000))>>8;
      Send_Data.tx[6]=(short)(arm_joint.data[2]*1000);

      Send_Data.tx[7]=(uint8_t)arm_joint.data[3];
...
void turn_on_robot::Publish_Voltage()
{
    ...
    if( ((Power_voltage>18&&Power_voltage<20)||(Power_voltage>8&&Power_voltage<10)) && Charging==0 && time_period>=2 )
    {
...
{ 
  ...
  */  
  ...
  if( ((Last_Receive== Distance_TAIL || Last_Receive == FRAME_TAIL) && Receive_Data_Pr[0] == AutoCharge_HEADER)||count3>0 )
    count3++;
  else
    count3=0;

  //接收到超声波的帧头、上一个24字节的帧尾，表明超声波数据开始到来
  if( (Last_Receive == FRAME_TAIL && Receive_Data_Pr[0]==Distance_HEADER) ||count2>0)
    count2++;
  else
    count2=0;

  //接受到24字节的帧头、自动回充数据的帧尾，表明24字节数据开始到来
  if(((Last_Receive == AutoCharge_TAIL || Last_Receive == FRAME_TAIL)&& Receive_Data_Pr[0] == FRAME_HEADER)||count>0) //Ensure that the first data in the array is FRAME_HEADER //确保数组第一个数据为FRAME_HEADER
    count++;
  else 
    count=0;
...
turn_on_robot::turn_on_robot():Sampling_Time(0),Power_voltage(0)
{
  ...
  private_nh.param<std::string>("arm_cmd", JointStates_topic, "arm_cmd");
  ...
  voltage_publisher = n.advertise<std_msgs::Float32>("power", 10); //Create a battery-voltage topic publisher //创建电池电压话题发布者
  ...
  SelfCheck_publisher = n.advertise<std_msgs::UInt32>("robot_selfcheck", 10); //自检数据话题发布者，避免未初始化发布导致崩溃
